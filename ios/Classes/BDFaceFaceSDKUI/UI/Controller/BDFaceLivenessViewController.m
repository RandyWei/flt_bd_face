...
//    _livenessArray = NULL;
    _order = false;
    _numberOfLiveness = 1; // _livenessArray=nil时有效
                ...
                if (images[@"image"] != nil && [images[@"image"] count] != 0) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [weakSelf dismissViewControllerAnimated:YES completion:^{
                            NSData *data = UIImageJPEGRepresentation(bestImage.cropImageWithBlack, 0.5f);
                            NSString *encodedImageStr = [data base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
//                            NSLog(@"===Encoded image:\n%@", encodedImageStr);
//                            weakSelf.faceImageAction(bestImage.cropImageWithBlackEncryptStr);
                            weakSelf.faceImageAction(encodedImageStr);
                            [self closeAction];
                        }];
                    });
                }
...
//                break;
            ...
            case LivenessRemindCodeFaceIdChanged:
            {
                ...
                [self warningStatus:CommonStatus warning:@"请把脸移入框内" conditionMeet:true];
            }
                break;
